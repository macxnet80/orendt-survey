...
const ADMIN_PW = process.env.NEXT_PUBLIC_ADMIN_PASSWORD || "orendt2026"
    ...
    <div className="min-h-screen bg-orendt-gray-50 flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl border border-orendt-gray-200 p-8 shadow-sm">
          ...
          <h2 className="font-display text-xl font-bold text-center mb-1">Dashboard Login</h2>
          <p className="text-sm text-orendt-gray-500 text-center mb-6">Passwort eingeben um fortzufahren</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            />
            {error && (
              <p className="text-red-500 text-xs mt-2 font-medium">Falsches Passwort</p>
            )}
            <button
              type="submit"
              className="w-full mt-4 px-4 py-3 rounded-xl bg-orendt-black text-orendt-accent font-display font-semibold text-sm tracking-wide hover:opacity-90 transition-opacity"
            >
              Anmelden
            </button>
          </form>
...
export default function AdminDashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    const { data: rData } = await getResponses()
    setQuestions(qData?.length ? qData : DEFAULT_QUESTIONS)
    setResponses(rData || [])
    setLoading(false)
  }, [])
    ...
    <div className="min-h-screen bg-orendt-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard
            label="Rücklaufquote"
            value={responses.length > 0 ? `${responses.length}` : "–"}
            sub="Teilnehmer insgesamt"
          />
          <StatCard
            label="Letzte Antwort"
            value={
              responses.length > 0
                ? new Date(responses[0].submitted_at).toLocaleDateString("de-DE", { day: "2-digit", month: "2-digit" })
                : "–"
            }
            sub={
              responses.length > 0
                ? new Date(responses[0].submitted_at).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })
                : ""
            }
          />
        </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {questions.map((q, i) => (
                <div
                  key={q.id}
                  className="bg-white rounded-2xl border border-orendt-gray-200 p-5 hover:border-orendt-gray-300 transition-colors group"
                >
                  <div className="flex items-start gap-4">
                    {/* Number */}
                    <div className="w-10 h-10 rounded-xl bg-orendt-gray-100 flex items-center justify-center flex-shrink-0">
                      <span className="font-display font-bold text-sm text-orendt-gray-600">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className={`px-2.5 py-0.5 rounded-md text-[11px] font-semibold font-display border ${TYPE_COLORS[q.type]}`}>
                          {TYPE_LABELS[q.type]}
                        </span>
                        <span className="text-[11px] font-display font-semibold tracking-wider uppercase text-orendt-gray-400">
                          {q.category}
                        </span>
                        {q.is_required && (
                          <span className="text-[10px] font-display font-semibold text-red-400 uppercase">Pflicht</span>
                        )}
                      </div>
                      <p className="font-body text-sm text-orendt-black font-medium leading-snug">{q.question}</p>
                      {q.options?.length > 0 && (
                        <p className="text-xs text-orendt-gray-400 mt-1">
                          {q.options.length} Optionen: {q.options.slice(0, 3).join(", ")}
                          {q.options.length > 3 && ` +${q.options.length - 3} mehr`}
                        </p>
                      )}
                      {q.rating_items?.length > 0 && (
                        <p className="text-xs text-orendt-gray-400 mt-1">
                          {q.rating_items.length} Kriterien
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-2 rounded-lg text-orendt-gray-400 hover:text-orendt-black hover:bg-orendt-gray-100 transition-colors"
                        title="Bearbeiten"
                      >
                        {icons.edit}
                      </button>
                      <button
                        ...
                        className="p-2 rounded-lg text-orendt-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors"
                        title="Löschen"
                      >
                        {icons.trash}
                      </button>
                  ...
                  return (
                    <div key={q.id} className="bg-white rounded-2xl border border-orendt-gray-200 p-6">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-semibold font-display border ${TYPE_COLORS[q.type]}`}>
                          {TYPE_LABELS[q.type]}
                        </span>
                        <span className="text-[11px] font-display font-semibold tracking-wider uppercase text-orendt-gray-400">
                          {q.category}
                        ...
                        <div className="space-y-2">
                          {Object.entries(s.avgs).map(([item, avg]) => (
                            <div key={item} className="flex items-center gap-3">
                              ...
                              <div className="flex-1 h-6 bg-orendt-gray-100 rounded-lg overflow-hidden">
                                <div
                                  className="h-full bg-orendt-accent rounded-lg transition-all duration-500"
                                  style={{ width: `${avg !== "–" ? (avg / 5) * 100 : 0}%` }}
                                />
                              </div>
                              <span className="text-sm font-display font-bold text-orendt-black w-10 text-right">
                                {avg}
                              </span>
                            </div>
                          ))}
                      ...
                      {s.type === "text" && s.texts.length > 0 && (
                        <div className="space-y-2 max-h-60 overflow-y-auto">
                          {s.texts.map((text, i) => (
                            <div key={i} className="px-4 py-3 bg-orendt-gray-50 rounded-xl text-sm text-orendt-gray-700 border border-orendt-gray-100">
                              &ldquo;{text}&rdquo;
                            </div>
                          ))}
                        </div>
                      ...
                      {s.type === "text" && s.texts.length === 0 && (
                        <p className="text-xs text-orendt-gray-400 italic">Keine Antworten</p>
                      )}
                    </div>
                  )
                })}
                ...
                <div className="mt-8">
                  ...
                  <div className="space-y-2">
                    {responses.map((r, i) => (
                      <div
                        key={r.id}
                        className="bg-white rounded-xl border border-orendt-gray-200 overflow-hidden"
                      >
                        <button
                          onClick={() => setExpandedResponse(expandedResponse === r.id ? null : r.id)}
                          className="w-full px-5 py-3.5 flex items-center justify-between text-left hover:bg-orendt-gray-50 transition-colors"
                        >
                          <div className="flex items-center gap-3">
                            <span className="font-display text-xs font-bold text-orendt-gray-400">
                              #{responses.length - i}
                            </span>
                          ...
                          <span className="text-orendt-gray-400">
                        ...
                        {expandedResponse === r.id && (
                          <div className="px-5 pb-4 border-t border-orendt-gray-100 pt-3 space-y-3 animate-fade-in">
                            {questions.map((q) => {
                              ...
                              return (
                                <div key={q.id}>
                                  <p className="text-[11px] font-display font-semibold tracking-wider uppercase text-orendt-gray-400 mb-0.5">
                                    {q.category}
                                  </p>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-sm text-orendt-gray-600 mb-6">
              Diese Aktion kann nicht rückgängig gemacht werden. Bestehende Antworten zu dieser Frage bleiben erhalten.
            </p>
