...
export async function getSurveys() {
  ...
  const active = data.filter((row) => !isSurveyExpired(row.expires_at))
...
export async function getSurveyBySlug(slug) {
  ...
  const { data, error } = await supabase
    ...
    .single()
  if (error || !data) return { data, error }
  if (isSurveyExpired(data.expires_at)) {
    return {
      data: null,
      error: { code: "expired", message: "Umfrage abgelaufen.", expires_at: data.expires_at },
    }
  }
  return { data, error: null }
}

export async function createSurvey(title, description = "", expires_at = null) {
  if (!supabase) return { data: null, error: "Supabase not configured" }
  ...
  const payload = { title, description, slug, owner_id }
...
/** Supabase/PostgREST liefert pro Request oft max. 1000 Zeilen — alle Seiten nachladen. */
...
export async function getResponses(surveyId) {
  ...
  for (;;) {
    ...
    const batch = data || []
    all.push(...batch)
    if (batch.length < RESPONSES_PAGE_SIZE) break
    from += RESPONSES_PAGE_SIZE
  }
